(function () {
  // Some mobile browsers / in-app webviews (Facebook, Instagram, custom tabs, etc.)
  // report an inaccurate `100vh`/`100dvh` because their address bar height isn't
  // reflected correctly. Setting a real pixel value from window.innerHeight and
  // keeping it updated fixes the extra blank space / cut-off FAB button issue.
...
(function () {
  ...
  document.addEventListener("click", (e) => {
    if (
      addBar.classList.contains("active") &&
      !addBar.contains(e.target) &&
      e.target !== fabBtn
    ) {
  ...
  function openReminderScreen(taskId, opts) {
    ...
    buildWheel(
      document.getElementById("minWheel"),
      minutes,
      pendingReminder.minute,
      (v) => {
    ...
    buildWheel(
      document.getElementById("ampmWheel"),
      ampms,
      pendingReminder.ampm === "AM" ? 0 : 1,
      (v) => {
